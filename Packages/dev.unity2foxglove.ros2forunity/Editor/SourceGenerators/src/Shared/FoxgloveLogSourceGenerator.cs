    ...
    public sealed class FoxgloveLogSourceGenerator : IIncrementalGenerator
    {
            ...
            System.Threading.CancellationToken token)
        {
            ...
            FoxRunRoslynTypeShapeBuilder.TryBuild(
                type,
                out var typeShape);
            var ros2MessageShape =
                FoxRunRoslynRos2MessageShapeBuilder.Build(
                    type,
                    context.SemanticModel.Compilation);
            var packagedCollection =
                IsTopLevelPackagedRos2MessageCollection(
                    type,
                    context.SemanticModel.Compilation);
            var ros2CustomDtoShape =
                !ros2MessageShape.ImplementsRos2Message
                && !packagedCollection
                    ? FoxRunRoslynRos2CustomDtoShapeBuilder
                        .Build(
                            type,
                            context.SemanticModel.Compilation)
                    : null;
            if (!ros2MessageShape.ImplementsRos2Message
                && !packagedCollection)
            {
                ...
                Compilation compilation)
        {
            ...
            return FoxRunRoslynRos2MessageShapeBuilder
                .Build(elementType, compilation)
