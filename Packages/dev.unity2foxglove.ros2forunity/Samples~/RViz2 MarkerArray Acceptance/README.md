...
The marker frame is fixed directly in `map`. The sample also publishes one minimal `/tf` transform, `map -> phase130_marker_origin`, so RViz2's Fixed Frame can resolve `map` without a persistent fixed-frame warning:

```text
frame_id = map
tf = map -> phase130_marker_origin
topic = /markers
...
5. Record runtime root, runtime root package flag, asset runtime flag, TF count, marker action, marker id, active marker count, and publish count in the evidence template.
...
```text
python Scripts\smoke\ros2\phase130_markerarray_acceptance.py --ros2-root ros2-windows\ros2_jazzy --rviz-config "Packages\dev.unity2foxglove.ros2forunity\Samples~\RViz2 MarkerArray Acceptance\rviz2_phase130_markerarray.rviz"
```
...
```text
python Scripts\smoke\ros2\launch_phase130_rviz2.py --ros2-root ros2-windows\ros2_jazzy --rviz-config "Packages\dev.unity2foxglove.ros2forunity\Samples~\RViz2 MarkerArray Acceptance\rviz2_phase130_markerarray.rviz"
```
...
- `/tf` has a publisher endpoint from `unity2foxglove_phase130_markerarray` and echoes `map -> phase130_marker_origin`.
