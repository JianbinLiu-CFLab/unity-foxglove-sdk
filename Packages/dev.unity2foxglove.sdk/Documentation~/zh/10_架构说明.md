...
Client → subscribeParameterUpdates / unsubscribeParameterUpdates (JSON)
  ...
  → 当已注册参数发生变化时，向订阅客户端广播 `parameterValues`。
```
...
- Client disconnect → 移除该客户端的 pending calls，因为断开后已无法再投递响应
...
- `serverInfo.capabilities` 包含 `parameters`、`parametersSubscribe`、`services`、`time`、`connectionGraph` 和 `clientPublish`；`assets` 与 `playbackControl` 只在运行时启用对应能力后声明
- `serverInfo.supportedEncodings` 包含 `json`，并在 protobuf schema 支持可用时包含 `protobuf`
...
- **Timestamp strategy:** `FoxgloveTimeUtil.NowUnixTimeNs()` uses `Stopwatch.GetTimestamp()` with UTC epoch anchor for nanosecond precision. `SystemClock.NowNs` delegates to the same source. `serverInfo` 会声明 `time` capability，`FoxgloveSession.BroadcastTime()` 会按节流频率发布二进制 time frames。
