...
The core SDK Package Manager entry provides three prepared samples:
...
Use **Basic Visualization** for the smallest first check. Use **Full Demo Visualization** for the full Foxglove WebSocket feature tour. Use **Virtual LiDAR Maze Demo** for a procedural point-cloud example on the core WebSocket path. The separate `Unity2Foxglove/` project is the maintainer/developer test project used for SDK development, manual acceptance, and IL2CPP build validation.

ROS transports and their samples live in opt-in companion packages. Install
`dev.unity2foxglove.ros2bridge` to get **ROS2 Bridge Sample**, or
`dev.unity2foxglove.ros2forunity` for the native R2FU samples.
...
| Procedural point-cloud demo                  | Virtual LiDAR Maze Demo   | Builds a controllable maze vehicle and streams its point cloud through the core WebSocket transport.              |
...
Start with **Basic Visualization** if you only want to know whether the package works. Use **Full Demo Visualization** when you want to explore the Foxglove WebSocket workflow. Install the relevant companion package before following a ROS-specific sample.
