...
namespace Unity.FoxgloveSDK.Editor
{
    ...
    public static class FoxRunRos2InterfacePackageRenderer
    {
        ...
        private static IEnumerable<FoxRunGenerationMember> SelectContracts(FoxRunGenerationModel model)
        {
            foreach (var member in model.Types.SelectMany(type => type.Members))
            {
                if (member.Ros2ContractKind != FoxRunRos2ContractKind.CustomDto
                ...
                {
                    throw new FoxRunRos2InterfaceRenderException(
                        "Custom ROS2 contract " + member.DeclaringType + "." + member.MemberName
        ...
        private static IReadOnlyDictionary<string, FoxRunRos2CustomDtoShape> CollectShapes(
            IReadOnlyList<FoxRunGenerationMember> contracts)
        {
        ...
        private static string ContractKey(FoxRunGenerationMember member)
