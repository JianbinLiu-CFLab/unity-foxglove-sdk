...
// Purpose: Schema-neutral native PackedPointCloud payload handoff for optional DDS publishers.
...
{
    /// <summary>
    /// Prepared PackedPointCloud-compatible payload metadata and packed point bytes.
    /// This type intentionally has no optional Provider dependency.
    /// </summary>
    public sealed class PackedPointCloudFrame
    {
        /// <summary>Create a prepared PackedPointCloud frame handoff.</summary>
        public PackedPointCloudFrame(
            ulong unixNs,
            string frameId,
            uint height,
            uint width,
            ...
            bool preferPooledDataRetention = false)
        {
            ...
            if (width != 0U && pointStep > uint.MaxValue / width)
            {
                throw new ArgumentOutOfRangeException(
                    nameof(width),
                    "PackedPointCloud row step must fit in a uint.");
            }
            ...
            if ((ulong)data.Length != expectedBytes)
            {
                throw new ArgumentException(
                    "PackedPointCloud data length must equal height * width * point_step.",
                    nameof(data));
            }
        ...
        /// <summary>Frame id written into the PackedPointCloud header.</summary>
        public string FrameId { get; }

        /// <summary>PackedPointCloud height. Phase 138L emits unorganized clouds with height 1.</summary>
        public uint Height { get; }

        /// <summary>PackedPointCloud width, equal to the published point-slot count for unorganized clouds.</summary>
