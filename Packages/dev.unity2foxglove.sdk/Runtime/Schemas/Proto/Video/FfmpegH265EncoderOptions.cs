...
namespace Foxglove.Schemas.Video
{
    ...
    public sealed class FfmpegH265EncoderOptions
    {
        ...
        public bool Validate(out string error)
        {
            ...
            if (!CameraVideoFrameGeometry.ValidateRgb24Dimensions(Width, Height, "FFmpeg H.265 RGB24", out error))
                return false;

            if (FrameRate <= 0)
            {
                error = "FFmpeg H.265 frame rate must be positive.";
                return false;
            }

            if (BitrateKbps <= 0)
            {
                error = "FFmpeg H.265 bitrate must be positive.";
                return false;
            }

            if (KeyframeInterval <= 0)
            {
                error = "FFmpeg H.265 keyframe interval must be positive.";
                return false;
            }

            if (MaxInputQueue <= 0 || MaxOutputQueue <= 0)
            {
                error = "FFmpeg H.265 queue sizes must be positive.";
                return false;
            }
        ...
        public ProcessStartInfo CreateStartInfo()
        {
            if (!Validate(out var error))
                throw new ArgumentException(error, nameof(FfmpegH265EncoderOptions));
        ...
        private static bool ValidatePreset(string value, out string error)
        {
            ...
            foreach (var c in preset)
            {
                if (c < ' ' || c == '\u007f')
                {
                    error = "FFmpeg H.265 preset contains control characters.";
                    return false;
                }
            }
            ...
            {
                error = "FFmpeg H.265 preset must be one of the known x265 preset names.";
                return false;
            }
