...
namespace Unity.FoxgloveSDK.IO
{
    public class McapRecorder : IDisposable
    {
        ...
        public McapRecorder(Stream stream, IFoxgloveLogger logger = null, int chunkSizeBytes = DefaultChunkSizeBytes)
        {
        ...
        public void AddChannel(uint fId, string topic, string enc, string sName, string sEnc, string sContent)
        {
            ...
            _w.WriteChannel(mCid, sid, topic, enc, new Dictionary<string, string>());
            _channels.Add(new ChannelRec { Id = mCid, Sid = sid, Topic = topic, Enc = enc });
        }
        ...
        public void Close()
        {
            ...
            foreach (var c in _channels) _w.WriteChannel(c.Id, c.Sid, c.Topic, c.Enc, new Dictionary<string, string>());
            _w.WriteStatistics(_msgCount, (ushort)_schemas.Count, (uint)_channels.Count, 0, 0, (uint)_chunkCount, _msgSt, _msgEt,
                _chMap.ToDictionary(kv => kv.Value.McapId, kv => (ulong)kv.Value.Seq));
            foreach (var ci in _chunkIdx)
                _w.WriteChunkIndex(ci.St, ci.Et, ci.Off, ci.Len, ci.Mio, ci.MioLen, "", ci.CompSz, ci.UncompSz);
            var sumOffStart = (ulong)_w.Position;
            _w.WriteSummaryOffset(0x02, sumStart, sumOffStart - sumStart);
        ...
        void FlushChunk()
        {
            ...
            var b = _chunkBuf.ToArray(); _chunkBuf.SetLength(0);
            var off = (ulong)_w.Position;
            _w.WriteChunk(_chunkSt, _chunkEt, (ulong)b.Length, 0, "", (ulong)b.Length, b);
            ...
            _chunkIdx.Add(new ChunkIdx { St = _chunkSt, Et = _chunkEt, Off = off, Len = chunkLen, Mio = mio, MioLen = mioTLen, CompSz = (ulong)b.Length, UncompSz = (ulong)b.Length });
        ...
        struct ChannelRec { public ushort Id, Sid; public string Topic, Enc; }
        struct ChunkIdx { public ulong St, Et, Off, Len, MioLen, CompSz, UncompSz; public Dictionary<ushort, ulong> Mio; }
    }
}
