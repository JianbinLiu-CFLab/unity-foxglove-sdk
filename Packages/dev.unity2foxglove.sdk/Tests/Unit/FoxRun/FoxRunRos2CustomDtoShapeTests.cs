...
namespace Unity.FoxgloveSDK.Tests.FoxRun
{
    public sealed class FoxRunRos2CustomDtoShapeTests
    {
        ...
        public void ReflectionBuilderRejectsReservedAndLossyDtoMembers()
        {
            ...
            Assert.Contains(diagnostics, value => value.StartsWith("FOXRUN606|", StringComparison.Ordinal));
        ...
        {
            yield return new object[] { typeof(GenericDto<int>), "FOXRUN606" };
            yield return new object[] { typeof(NonConstructibleDto), "FOXRUN607" };
            yield return new object[] { typeof(NonWritableDto), "FOXRUN608" };
            yield return new object[] { typeof(CyclicDto), "FOXRUN606" };
            yield return new object[] { typeof(UnsupportedCollectionDto), "FOXRUN606" };
            yield return new object[] { typeof(UnsupportedReferenceDto), "FOXRUN606" };
            yield return new object[] { typeof(UnsupportedArrayDto), "FOXRUN606" };
        }
        ...
        [Fact]
        public void RoslynAndReflectionBuildersProduceTheSameCustomDtoSchema()
        {
            const string source = @"
namespace Unity.FoxgloveSDK.Tests.FoxRun
{
    public sealed class FoxRunRos2CustomDtoShapeTests
    {
        ...
        public sealed class NestedDto
        {
            ...
            var compilation = CSharpCompilation.Create(
                "CustomDtoShapeParity",
                new[] { CSharpSyntaxTree.ParseText(source) },
                TrustedPlatformReferences(),
                new CSharpCompilationOptions(OutputKind.DynamicallyLinkedLibrary));
            var roslynType = compilation.GetTypeByMetadataName(
                "Unity.FoxgloveSDK.Tests.FoxRun.FoxRunRos2CustomDtoShapeTests+ParityDto");
            Assert.NotNull(roslynType);

            var roslyn = FoxRunRoslynRos2CustomDtoShapeBuilder.Build(roslynType, compilation);
            var reflection = FoxRunReflectionRos2CustomDtoShapeBuilder.Build(typeof(ParityDto));

            Assert.True(
                reflection.IsSupported == roslyn.IsSupported,
                "Roslyn diagnostics: " + string.Join(" || ", roslyn.Diagnostics));
            Assert.Equal(reflection.CanonicalIdentity, roslyn.CanonicalIdentity);
            Assert.Equal(reflection.PayloadIdentity, roslyn.PayloadIdentity);
            Assert.Equal(reflection.Diagnostics, roslyn.Diagnostics);
            Assert.Equal(
                reflection.Members.Select(member => MemberEvidence(member)).ToArray(),
                roslyn.Members.Select(member => MemberEvidence(member)).ToArray());
        }

        [Fact]
        public void RoslynAndReflectionBuildersProduceTheSameUnsupportedDiagnostics()
        {
            const string source = @"
using System.Collections.Generic;
namespace Unity.FoxgloveSDK.Tests.FoxRun
{
    public sealed class FoxRunRos2CustomDtoShapeTests
    {
        public sealed class ParityUnsupportedDto
        {
            ...
            var compilation = CSharpCompilation.Create(
                "CustomDtoUnsupportedParity",
                new[] { CSharpSyntaxTree.ParseText(source) },
                TrustedPlatformReferences(),
                new CSharpCompilationOptions(OutputKind.DynamicallyLinkedLibrary));
            var roslynType = compilation.GetTypeByMetadataName(
                "Unity.FoxgloveSDK.Tests.FoxRun.FoxRunRos2CustomDtoShapeTests+ParityUnsupportedDto");
            Assert.NotNull(roslynType);

            var roslyn = FoxRunRoslynRos2CustomDtoShapeBuilder.Build(roslynType, compilation);
            var reflection = FoxRunReflectionRos2CustomDtoShapeBuilder.Build(typeof(ParityUnsupportedDto));
