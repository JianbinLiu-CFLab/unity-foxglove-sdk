...
| Phase150 | Schemas | `Schemas/ProtoSchemaCatalogTests` | partial | Pure CLR-type protobuf catalog lookup behavior is covered in xUnit. Manager channel facade/source-shape and Unity-facing API boundaries stay in the console runner. |
| Phase179 | FoxRun native ROS2 subscriptions | `FoxRun/FoxRunSubscriptionProviderResolverTests`, `FoxRun/FoxRunRos2BindingGenerationTests`, `FoxRun/FoxRunInboundTests`, `Ros2ForUnity/FoxRunRos2NativeSubscriptionAdapterTests` | partial | Pure provider, diagnostic, router coexistence, generated-binding, latest-wins, and optional-runtime behavior run in xUnit. Repository/CI wiring, public sample/Player builder shape, and Linux/Unity Editor/Player interoperability remain in the console runner or local manual matrix. |
