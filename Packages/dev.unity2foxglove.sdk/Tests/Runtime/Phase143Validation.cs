...
{
    /// <summary>
    /// Validation type for Phase143Validation.
    /// </summary>
            ...
            var offenders = productionRoots
                ...
                .SelectMany(path =>
                {
                    ...
                    var forbidden = Path.GetExtension(path).Equals(".cs", StringComparison.OrdinalIgnoreCase)
                        ? PhaseRos2ForUnityValidationHelpers.FindHardR2fuCSharpIdentifiers(text, CoreForbiddenTokens())
                        : CoreForbiddenTokens().Where(token => text.Contains(token, StringComparison.Ordinal));
                    return forbidden
