...
namespace Unity2Foxglove.Ros2Bridge
{
        ...
        IFoxRunMcapDecoderContribution
    {
            ...
            out string reason)
        {
            ...
            else if (request.Value is PointCloud2NativeFrame pointCloud2)
            {
                if (!MatchesLogicalType<PointCloud2NativeFrame>(
                        request.LogicalSchemaName))
                {
                    return RejectLogicalType(
                        request.LogicalSchemaName,
                        typeof(PointCloud2NativeFrame),
                        out contribution,
                        out reason);
                }
