...
namespace Unity.FoxgloveSDK.Tests.Unit.FoxRun
{
    public sealed class FoxRunRos2BridgeQosMapperTests
    {
        ...
        [Fact]
        public void BridgeProviderUsesNeutralLifecycleWithoutLegacyOutputSwitch()
        {
            var provider = ReadRepoText(
                "Packages/dev.unity2foxglove.ros2bridge/Runtime/Ros2Bridge/Ros2BridgeTransportProvider.cs");
        ...
        [Fact]
        public void BridgeSessionStartsRuntimeBeforePublishingActiveState()
        {
            var provider = ReadRepoText(
                "Packages/dev.unity2foxglove.ros2bridge/Runtime/Ros2Bridge/Ros2BridgeTransportProvider.cs");
            ...
            var capture = root.DescendantNodes()
                .OfType<MethodDeclarationSyntax>()
                .Single(method =>
                    method.Identifier.ValueText == "TryCaptureSession");
            var body = capture.Body?.ToFullString() ?? string.Empty;

            var availability = body.IndexOf(
                "!isActiveAndEnabled || !_available",
                StringComparison.Ordinal);
            var runtimeStart = body.IndexOf(
                "runtime.Start(",
                StringComparison.Ordinal);
            var publishSession = body.IndexOf(
                "_activeSession = captured",
                StringComparison.Ordinal);

            Assert.True(
                availability >= 0
                && availability < runtimeStart
                && runtimeStart < publishSession);
            Assert.Contains(
        ...
        public void EndingBridgeSessionAlwaysReleasesProviderOwnership()
        {
            ...
            var method = CSharpSyntaxTree.ParseText(
                ...
                .Single(candidate =>
                    ...
                    && candidate.ToFullString().Contains(
                        "ReleaseSession",
                        StringComparison.Ordinal));
            var body = method.Body?.ToFullString() ?? string.Empty;
        ...
        [Fact]
        public void DirectAndCapturedBridgeMappersUseOneCanonicalMappingCore()
        {
            var provider = ReadRepoText(
                "Packages/dev.unity2foxglove.ros2bridge/Runtime/Ros2Bridge/Ros2BridgeTransportProvider.cs");
            var root = CSharpSyntaxTree.ParseText(
                    ...
                    new CSharpParseOptions(
                        preprocessorSymbols: new[] { "UNITY_5_3_OR_NEWER" }))
                .GetCompilationUnitRoot();
            var mappingCore = root.DescendantNodes()
                .OfType<MethodDeclarationSyntax>()
                .Single(method =>
                    method.Identifier.ValueText == "TryMapOrdinary");
            var publicMappers = root.DescendantNodes()
                .OfType<MethodDeclarationSyntax>()
                .Where(method =>
                    method.Identifier.ValueText == "TryMap"
                    && method.ParameterList.Parameters.Any(parameter =>
                        parameter.Type?.ToString().Contains(
                            "FoxRunOrdinaryPayloadRequest",
                            StringComparison.Ordinal) == true))
                .ToArray();

            Assert.Equal(2, publicMappers.Length);
            Assert.All(
                publicMappers,
            ...
            Assert.Contains(
                ...
                mappingCore.ToFullString(),
                StringComparison.Ordinal);
        }

        [Fact]
        public void ProviderLifecyclePublishesAndClearsOneManagerLocalSession()
        {
            var provider = ReadRepoText(
                "Packages/dev.unity2foxglove.ros2bridge/Runtime/Ros2Bridge/Ros2BridgeTransportProvider.cs");
        ...
        [Fact]
        public void OrdinaryPublishersReachBridgeOnlyThroughNeutralProviderFanout()
        {
            ...
            Assert.Contains(
                ...
                StringComparison.Ordinal);
            Assert.Contains(
                "_manager.PublishOrdinaryTransports(in request)",
                publisherBase,
                StringComparison.Ordinal);

            var roots = new[]
            {
                "Packages/dev.unity2foxglove.sdk/Runtime/Components/Publishing",
                "Packages/dev.unity2foxglove.sdk/Runtime/Schemas/Camera",
                "Packages/dev.unity2foxglove.sdk/Runtime/Schemas/PointCloud",
            };
