...
#
# Module: Scripts/ros2forunity/windows/jazzy
...
DEFAULT_BUILD_ROOT = pathlib.Path(__file__).resolve().parents[4] / "build" / "r2fu-jazzy-win64"
...
def parse_args(argv: list[str]) -> argparse.Namespace:
    ...
    parser.add_argument("--ros2-root", default=str(default_ros2_root("jazzy")))
...
def run_command(
    ...
    *,
    ...
    try:
        ...
        reader = threading.Thread(target=read_output, name="phase138b-command-output", daemon=True)
        reader.start()
        with log_file.open("a", encoding="utf-8", errors="replace") as log:
            while True:
