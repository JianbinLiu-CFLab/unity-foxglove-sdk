...
# Humble Win64 runtime package and record local evidence.
    ...
    if foreign_runtime_packages:
        if not update:
            ...
            raise RuntimeError(
                ...
                "rerun with --update-project-manifest to switch to Humble."
            )
...
def run_unity_import(unity_exe: Path, project_path: Path, log_path: Path) -> None:
    ...
    clean_env["PATH"] = ";".join(
        ...
        if "ros2_humble" not in item.lower()
...
def main(argv: list[str] | None = None) -> int:
    ...
    inventory_path = project_root / "Packages" / "dev.unity2foxglove.ros2forunity" / "Compliance" / "r2fu-humble-win64-runtime-inventory.json"
