...
def inspect_zip(paths: ArtifactPaths) -> dict[str, object]:
    ...
    return {
        ...
        "knownCaveats": [
            ...
            "The Humble package must include transitive DLLs such as yaml.dll, spdlog.dll, and fmt.dll or Unity may fail to load rcl.dll.",
            "Windows Firewall may block inbound Fast DDS UDP discovery; configure allow rules for DDS ports. Windows ROS2 Humble, properly firewalled WSL2, or a real remote Linux topology should be used for acceptance.",
        ],
        "files": files,
    }
