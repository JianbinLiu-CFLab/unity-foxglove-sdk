...
# Usage: python Scripts/ros2forunity/windows/humble/validate_ros2forunity_package.py
...
RUNTIME_INVENTORY = PACKAGE / "Compliance" / "r2fu-humble-win64-runtime-inventory.json"
RUNTIME_PACKAGE_NAME = "dev.unity2foxglove.ros2forunity.runtime.humble.win64"
RUNTIME_PACKAGE = ROOT / "Packages" / RUNTIME_PACKAGE_NAME
...
RUNTIME_SHA256 = "6937f348b2abdf40614379173bb81ba55090dc1541cab616d1a0f1e248ceb5b0"
...
def get_humble_runtime(data: dict) -> dict:
    """Return the Humble supported runtime entry from the adoption manifest."""
    packages = data.get("supportedRuntimePackages", [])
    if not isinstance(packages, list):
        return {}
    for package in packages:
        if isinstance(package, dict) and package.get("packageName") == RUNTIME_PACKAGE_NAME:
            return package
    return {}
...
def check_manifest(results: list[CheckResult]) -> None:
    ...
    add(
        results,
        "manifest known runtime distros include humble",
        "humble" in {part.strip() for part in known_distros.split(",")},
        f"knownRuntimeRosDistro={known_distros!r}",
    )

    current = get_humble_runtime(data)
    add(
        results,
        "manifest supported Humble runtime object",
        isinstance(current, dict),
        f"supportedRuntimePackages={data.get('supportedRuntimePackages')!r}",
    )
    if isinstance(current, dict):
        current_expected = {
            ...
            "inventoryFile": "Compliance/r2fu-humble-win64-runtime-inventory.json",
            "runtimeNoticesFile": f"Packages/{RUNTIME_PACKAGE_NAME}/THIRD_PARTY_NOTICES.md",
        }
        for key, value in current_expected.items():
            add(
                results,
                f"manifest supported Humble runtime {key}",
        ...
        add(
            results,
            "manifest supported Humble runtime artifactSha256",
            artifact_sha == RUNTIME_SHA256,
            f"artifactSha256={artifact_sha!r}",
        )
        add(
            results,
            "manifest supported Humble runtime artifactSize",
            isinstance(artifact_size, int) and artifact_size > 0,
            f"artifactSize={artifact_size!r}",
        )
        add(
            results,
            "manifest supported Humble runtime inventoryFileCount",
        ...
        add(
            results,
            "manifest supported Humble runtime critical files",
            isinstance(critical_files, list)
            and {"rcl.dll", "yaml.dll", "spdlog.dll", "rosgraph_msgs_assembly.dll"}.issubset(set(critical_files)),
    ...
    add(
        ...
        and RUNTIME_PACKAGE_NAME not in packages
...
def check_text_boundaries(results: list[CheckResult]) -> None:
    ...
    add(
        ...
        RUNTIME_ARTIFACT in runtime_notices
        and RUNTIME_SHA256 in runtime_notices,
        rel(RUNTIME_NOTICES),
    )
    add(
        results,
...
def check_runtime_inventory(results: list[CheckResult]) -> None:
    """Validate the generated Humble runtime artifact inventory."""
    ...
    current = get_humble_runtime(manifest_data) if isinstance(manifest_data, dict) else {}
    ...
    add(
        ...
        all(critical_map.get(name, {}).get("present") for name in ("rcl.dll", "yaml.dll", "spdlog.dll"))
        and critical_map.get("fmt.dll", {}).get("present") is False,
...
def check_sample_source_boundary(results: list[CheckResult]) -> None:
    ...
    add(
        ...
        "dev.unity2foxglove.ros2forunity.runtime.humble.win64" in readme
    ...
    standard_helper = (ROOT / "Scripts" / "smoke" / "ros2" / "phase132_standard_messages_acceptance.py")
    standard_helper_text = standard_helper.read_text(encoding="utf-8", errors="replace") if standard_helper.exists() else ""
    standard_launcher = (ROOT / "Scripts" / "smoke" / "ros2" / "launch_phase132_rviz2.py")
