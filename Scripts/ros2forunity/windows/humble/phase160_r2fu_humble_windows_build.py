...
#
# Module: Scripts/ros2forunity/windows/humble
# Purpose: Phase160 R2FU Humble Windows build toolchain orchestrator.

"""Run a deterministic R2FU Humble Windows build attempt.
...
R2FU_BRANCH = "feature/humble-support"
ROS2CS_REPO_URL = "https://github.com/RobotecAI/ros2cs.git"
ROS2CS_BRANCH = "feature/humble-support"
DEFAULT_BUILD_ROOT = pathlib.Path(__file__).resolve().parents[4] / "build" / "r2fu-humble-win64"
...
class Phase160Error(RuntimeError):
...
def parse_args(argv: list[str]) -> argparse.Namespace:
    ...
    parser.add_argument("--ros2-root", default=str(default_ros2_root("humble")))
    ...
    parser.add_argument("--log-prefix", default="phase160")
    parser.add_argument(
        "--evidence-path",
        default="",
...
def find_repo_root(start: pathlib.Path) -> pathlib.Path:
    ...
    for candidate in (current, *current.parents):
        if (candidate / ".git").exists():
            return candidate
    raise Phase160Error("BLOCKED_UNKNOWN_TOOLCHAIN", f"Could not locate repo root from {start}")
...
def remove_known_subdir(path: pathlib.Path, allowed_parent: pathlib.Path) -> None:
    ...
    if resolved == parent or not is_relative_to(resolved, parent):
        raise Phase160Error("BLOCKED_UNKNOWN_TOOLCHAIN", f"Refusing unsafe delete: {resolved}")
    if resolved.exists():
...
def run_command(
    ...
    *,
    ...
    if check and exit_code != 0:
        raise Phase160Error(classify_output(output), f"Command failed: {' '.join(command)}")
    return result
...
def find_vswhere() -> pathlib.Path:
    ...
    for candidate in candidates:
        if candidate.exists():
            return candidate
    raise Phase160Error("BLOCKED_VSDEV_ENV", "vswhere.exe was not found")
...
def resolve_vs_dev_cmd(explicit: str, env: dict[str, str], log_file: pathlib.Path) -> pathlib.Path:
    ...
    if explicit:
        ...
        if path.exists():
            return path
        raise Phase160Error("BLOCKED_VSDEV_ENV", f"Explicit VsDevCmd.bat was not found: {path}")
    ...
    for candidate in candidates:
        if candidate.exists():
            return candidate
    raise Phase160Error("BLOCKED_VSDEV_ENV", "VsDevCmd.bat was not found")
...
def scrub_environment(env: dict[str, str], ros2_root: pathlib.Path, temp_root: pathlib.Path) -> dict[str, str]:
    """Remove common Python contamination and pin ROS2 Humble paths."""
...
def capture_ros2_environment(ros2_root: pathlib.Path, env: dict[str, str], log_file: pathlib.Path) -> dict[str, str]:
    ...
    if not setup.exists():
        raise Phase160Error("BLOCKED_PYTHON_SELECTION", f"ROS2 local_setup.ps1 was not found: {setup}")
...
def verify_python(ros2_root: pathlib.Path, env: dict[str, str], log_file: pathlib.Path) -> pathlib.Path:
    """Verify the pinned ROS2 Humble pixi Python can import catkin_pkg."""

    python = ros2_root / ".pixi" / "envs" / "default" / "python.exe"
    if not python.exists():
        raise Phase160Error("BLOCKED_PYTHON_SELECTION", f"Pinned Python is missing: {python}")
    run_command(
...
def verify_cl_compile(temp_root: pathlib.Path, env: dict[str, str], log_file: pathlib.Path) -> None:
    ...
    source = temp_root / "phase160_cl_probe.c"
    obj = temp_root / "phase160_cl_probe.obj"
    source.write_text("int main(void) { return 0; }\n", encoding="utf-8")
    cl_path = shutil.which("cl", path=env.get("Path") or env.get("PATH"))
    if not cl_path:
        raise Phase160Error("BLOCKED_VSDEV_ENV", "cl.exe was not found in the imported VS environment")
    try:
        result = run_command(
        ...
        if result.exit_code != 0:
            raise Phase160Error(classify_output(result.output), "cl.exe tiny compile failed")
    finally:
...
def resolve_cmake_generator(requested: str, env: dict[str, str], log_file: pathlib.Path) -> str:
    ...
    if requested == "ninja" and not ninja:
        raise Phase160Error("BLOCKED_CMAKE_GENERATOR", "Ninja was requested but ninja.exe was not found")
...
def ensure_r2fu_checkout(work_root: pathlib.Path, env: dict[str, str], log_file: pathlib.Path, clean: bool) -> pathlib.Path:
    """Create or update a clean R2FU Humble-support checkout under WorkRoot."""
...
def patch_ros2cs_humble_windows_standalone(checkout: pathlib.Path, log_file: pathlib.Path) -> None:
    """Patch the checkout so Humble Windows standalone uses OpenSSL 3 DLL names."""
...
"""
    if "libssl-3-x64.dll" in text and "ros2_distro STREQUAL \"humble\"" in text:
        patched = False
    else:
        pattern = re.compile(
    ...
    with log_file.open("a", encoding="utf-8", errors="replace") as log:
        ...
        log.write(f"\n# patch_ros2cs_humble_windows_standalone={state}\n")


def run_humble_dependency_import(checkout: pathlib.Path, env: dict[str, str], log_file: pathlib.Path) -> None:
    """Run the upstream Humble dependency import."""

    ros2cs = checkout / "src" / "ros2cs"
    get_repos = ros2cs / "get_repos.ps1"
    if not get_repos.exists():
        raise Phase160Error("BLOCKED_ROS2CS_BUILD", f"Missing upstream get_repos.ps1: {get_repos}")
    result = run_command(
    ...
    if result.exit_code != 0:
        raise Phase160Error(classify_output(result.output), "Humble dependency import failed")
...
def has_complete_asset(checkout: pathlib.Path) -> bool:
    ...
    required_runtime = all((plugins / name).exists() for name in HUMBLE_PIXI_RUNTIME_DLLS)
    return asset.exists() and metadata.exists() and plugins.exists() and bool(native_libraries) and required_runtime


def copy_humble_pixi_runtime_closure(
    asset: pathlib.Path,
    ...
    copied: list[str] = []
    for name in HUMBLE_PIXI_RUNTIME_DLLS:
        source = pixi_bin / name
        if not source.exists():
            raise Phase160Error("BLOCKED_NATIVE_DEPENDENCY", f"Missing Humble pixi runtime DLL: {source}")
        shutil.copy2(source, plugin_dir / name)
        copied.append(name)

    with log_file.open("a", encoding="utf-8", errors="replace") as log:
        log.write("\n# copied Humble pixi runtime closure DLLs\n")
        for name in copied:
...
def deploy_asset_from_successful_colcon(
    ...
    copy_humble_pixi_runtime_closure(asset, ros2_root, log_file)
...
def run_direct_colcon_build(
    ...
    if not colcon_path:
        raise Phase160Error("BLOCKED_PYTHON_SELECTION", "colcon was not found in the pinned ROS2 Humble environment")
    command = [
        colcon_path,
        "build",
        "--merge-install",
...
def run_upstream_build(
    ...
    if not build_script.exists():
        raise Phase160Error("BLOCKED_R2FU_BUILD_SCRIPT", f"Missing upstream build.ps1: {build_script}")
    result = run_command(
...
def write_evidence(
    ...
    *,
    ...
    """Write the local Phase160 evidence note."""

    lines = [
        "---",
        'title: "Phase160 R2FU Humble Windows Build Toolchain Closure Evidence"',
        "tags:",
        "  - developer",
        "  - phase160",
        "  - ros2-for-unity",
        ...
        "---",
        "",
        "# Phase160 R2FU Humble Windows Build Toolchain Closure Evidence",
        "",
        "## Verdict",
        "",
        "```text",
        ...
        "## Boundary",
        "",
        "Phase160 does not import any partial R2FU asset into Unity.",
        "",
...
def main(argv: list[str]) -> int:
    ...
    try:
        ...
        if args.no_build:
            ...
            patch_ros2cs_humble_windows_standalone(checkout, log_file)
            run_humble_dependency_import(checkout, env, log_file)
