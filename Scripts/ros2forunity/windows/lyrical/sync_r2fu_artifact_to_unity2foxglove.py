...
# Lyrical Win64 runtime package and record local evidence.
...
def run_unity_import(unity_exe: Path, project_path: Path, log_path: Path) -> None:
    ...
    clean_env["PATH"] = ";".join(
        ...
        if "ros2_lyrical" not in item.lower()
...
def main(argv: list[str] | None = None) -> int:
    ...
    inventory_path = project_root / "Packages" / "dev.unity2foxglove.ros2forunity" / "Compliance" / "r2fu-lyrical-win64-runtime-inventory.json"
    package_path = project_root / "Packages" / PACKAGE_NAME
    inspect_script = project_root / "Scripts" / "ros2forunity" / "windows" / "lyrical" / "inspect_r2fu_runtime_artifact.py"
    build_script = project_root / "Scripts" / "ros2forunity" / "windows" / "lyrical" / "build_r2fu_runtime_package.py"
    ...
    transaction = DurableSnapshot(
        ...
        project_root / "build" / "phase187-round4" / "scratch" / "r41-h04-009-sync-lyrical-before-image",
    )
