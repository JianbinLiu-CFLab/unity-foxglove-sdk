...
#
# Module: Scripts/ros2forunity/windows/lyrical
# Purpose: Phase146B R2FU Lyrical Windows build toolchain orchestrator.

"""Run a deterministic R2FU Lyrical Windows build attempt.
...
R2FU_BRANCH = "feature/lyrical-support"
ROS2CS_REPO_URL = "https://github.com/RobotecAI/ros2cs.git"
ROS2CS_BRANCH = "feature/lyrical-support"
DEFAULT_BUILD_ROOT = pathlib.Path(__file__).resolve().parents[4] / "build" / "r2fu-lyrical-win64"
DEFAULT_WORK_ROOT = DEFAULT_BUILD_ROOT / "work"
DEFAULT_TEMP_ROOT = DEFAULT_BUILD_ROOT / "tmp"
DEFAULT_EVIDENCE_PATH = pathlib.Path("build") / "phase146b" / "r2fu_lyrical_windows_build_evidence.md"
CHECKOUT_DIR_NAME = "r2u"
LYRICAL_PIXI_RUNTIME_DLLS = ("yaml.dll", "spdlog.dll", "fmt.dll")
...
class Phase146BError(RuntimeError):
...
def parse_args(argv: list[str]) -> argparse.Namespace:
    ...
    parser.add_argument("--log-prefix", default="phase146b")
    parser.add_argument(
        "--evidence-path",
        default="",
...
def remove_known_subdir(path: pathlib.Path, allowed_parent: pathlib.Path) -> None:
    ...
    if resolved == parent or not is_relative_to(resolved, parent):
        raise Phase146BError("BLOCKED_UNKNOWN_TOOLCHAIN", f"Refusing unsafe delete: {resolved}")
    if resolved.exists():
...
def run_command(
    ...
    *,
    ...
    if check and exit_code != 0:
        raise Phase146BError(classify_output(output), f"Command failed: {' '.join(command)}")
    return result
...
def find_vswhere() -> pathlib.Path:
    ...
    for candidate in candidates:
        if candidate.exists():
            return candidate
    raise Phase146BError("BLOCKED_VSDEV_ENV", "vswhere.exe was not found")
...
def resolve_vs_dev_cmd(explicit: str, env: dict[str, str], log_file: pathlib.Path) -> pathlib.Path:
    ...
    if explicit:
        ...
        if path.exists():
            return path
        raise Phase146BError("BLOCKED_VSDEV_ENV", f"Explicit VsDevCmd.bat was not found: {path}")
    ...
    for candidate in candidates:
        if candidate.exists():
            return candidate
    raise Phase146BError("BLOCKED_VSDEV_ENV", "VsDevCmd.bat was not found")
...
def capture_cmd_environment(vs_dev_cmd: pathlib.Path, env: dict[str, str], log_file: pathlib.Path) -> dict[str, str]:
    ...
    if completed.returncode != 0:
        raise Phase146BError(classify_output(output), f"Command failed: {command}")
    merged = dict(env)
...
def reject_cmd_shell_unsafe_path(label: str, path: pathlib.Path) -> None:
    ...
    if '"' in str(path):
        raise Phase146BError("BLOCKED_VSDEV_ENV", f"{label} contains an unsupported quote: {path}")
    if "%" in str(path):
        raise Phase146BError("BLOCKED_VSDEV_ENV", f"{label} contains unsupported percent expansion: {path}")


def scrub_environment(env: dict[str, str], ros2_root: pathlib.Path, temp_root: pathlib.Path) -> dict[str, str]:
    """Remove common Python contamination and pin ROS2 Lyrical paths."""
...
def verify_python(ros2_root: pathlib.Path, env: dict[str, str], log_file: pathlib.Path) -> pathlib.Path:
    """Verify the pinned ROS2 Lyrical pixi Python can import catkin_pkg."""

    python = ros2_root / ".pixi" / "envs" / "default" / "python.exe"
    if not python.exists():
        raise Phase146BError("BLOCKED_PYTHON_SELECTION", f"Pinned Python is missing: {python}")
    run_command(
...
def verify_cl_compile(temp_root: pathlib.Path, env: dict[str, str], log_file: pathlib.Path) -> None:
    ...
    source = temp_root / "phase146b_cl_probe.c"
    obj = temp_root / "phase146b_cl_probe.obj"
    source.write_text("int main(void) { return 0; }\n", encoding="utf-8")
    cl_path = shutil.which("cl", path=env.get("Path") or env.get("PATH"))
    if not cl_path:
        raise Phase146BError("BLOCKED_VSDEV_ENV", "cl.exe was not found in the imported VS environment")
    try:
        result = run_command(
        ...
        if result.exit_code != 0:
            raise Phase146BError(classify_output(result.output), "cl.exe tiny compile failed")
    finally:
...
def resolve_cmake_generator(requested: str, env: dict[str, str], log_file: pathlib.Path) -> str:
    ...
    if requested == "ninja" and not ninja:
        raise Phase146BError("BLOCKED_CMAKE_GENERATOR", "Ninja was requested but ninja.exe was not found")
...
def ensure_r2fu_checkout(work_root: pathlib.Path, env: dict[str, str], log_file: pathlib.Path, clean: bool) -> pathlib.Path:
    """Create or update a clean R2FU Lyrical-support checkout under WorkRoot."""
...
def patch_ros2cs_lyrical_windows_standalone(checkout: pathlib.Path, log_file: pathlib.Path) -> None:
    """Patch the checkout so Lyrical Windows standalone uses OpenSSL 3 DLL names."""
    ...
    if(ros2_distro STREQUAL "lyrical" OR ros2_distro STREQUAL "rolling")
...
"""
    if "libssl-3-x64.dll" in text and "ros2_distro STREQUAL \"lyrical\"" in text:
        patched = False
    else:
        pattern = re.compile(
    ...
    with log_file.open("a", encoding="utf-8", errors="replace") as log:
        ...
        log.write(f"\n# patch_ros2cs_lyrical_windows_standalone={state}\n")


def run_lyrical_dependency_import(checkout: pathlib.Path, env: dict[str, str], log_file: pathlib.Path) -> None:
    """Run the upstream Lyrical dependency import."""
    ...
    if result.exit_code != 0:
        raise Phase146BError(classify_output(result.output), "Lyrical dependency import failed")
...
def has_complete_asset(checkout: pathlib.Path) -> bool:
    ...
    required_runtime = all((plugins / name).exists() for name in LYRICAL_PIXI_RUNTIME_DLLS)
    return asset.exists() and metadata.exists() and plugins.exists() and bool(native_libraries) and required_runtime


def copy_lyrical_pixi_runtime_closure(
    asset: pathlib.Path,
    ...
    copied: list[str] = []
    for name in LYRICAL_PIXI_RUNTIME_DLLS:
        source = pixi_bin / name
        if not source.exists():
            raise Phase146BError("BLOCKED_NATIVE_DEPENDENCY", f"Missing Lyrical pixi runtime DLL: {source}")
        shutil.copy2(source, plugin_dir / name)
        copied.append(name)

    with log_file.open("a", encoding="utf-8", errors="replace") as log:
        log.write("\n# copied Lyrical pixi runtime closure DLLs\n")
        for name in copied:
...
def deploy_asset_from_successful_colcon(
    ...
    copy_lyrical_pixi_runtime_closure(asset, ros2_root, log_file)
...
def run_direct_colcon_build(
    ...
    if not colcon_path:
        raise Phase146BError("BLOCKED_PYTHON_SELECTION", "colcon was not found in the pinned ROS2 Lyrical environment")
    command = [
        colcon_path,
        "build",
        "--merge-install",
...
def write_evidence(
    ...
    *,
    ...
    """Write the local Phase146B evidence note."""

    lines = [
        "---",
        'title: "Phase146B R2FU Lyrical Windows Build Toolchain Closure Evidence"',
        "tags:",
        "  - developer",
        "  - phase146b",
        "  - ros2-for-unity",
        ...
        "---",
        "",
        "# Phase146B R2FU Lyrical Windows Build Toolchain Closure Evidence",
        "",
        "## Verdict",
        "",
        "```text",
        ...
        "## Boundary",
        "",
        "Phase146B does not import any partial R2FU asset into Unity.",
        "",
        "Live Unity/RViz2 acceptance remains a separate gate after a complete Lyrical runtime package exists.",
        "",
    ]
...
def main(argv: list[str]) -> int:
    ...
    try:
        ...
        if args.no_build:
            ...
            patch_ros2cs_lyrical_windows_standalone(checkout, log_file)
            run_lyrical_dependency_import(checkout, env, log_file)
