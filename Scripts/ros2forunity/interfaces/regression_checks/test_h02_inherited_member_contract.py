...
class H02019InheritedMemberContractTests(unittest.TestCase):
    ...
    def test_reflection_builder_walks_public_base_chain(self):
        ...
        self.assertIn('for (var current = type; current != null && current != typeof(object); current = current.BaseType)', source)
        self.assertIn('var seen = new HashSet<string>(StringComparer.Ordinal);', source)

    def test_roslyn_builder_walks_public_base_chain(self):
        """R4.1 regression contract."""
        source = ROSLYN.read_text(encoding='utf-8')
        self.assertIn('for (var current = type; current != null && current.SpecialType != SpecialType.System_Object; current = current.BaseType)', source)
        self.assertIn('var seen = new HashSet<string>(StringComparer.Ordinal);', source)

    def test_reflection_message_builder_walks_public_base_chain(self):
        """R4.1 regression contract."""
        source = REFLECTION_MSG.read_text(encoding='utf-8')
        self.assertIn('for (var current = type; current != null && current != typeof(object); current = current.BaseType)', source)
        self.assertIn('var seen = new HashSet<string>(StringComparer.Ordinal);', source)

    def test_roslyn_message_builder_walks_public_base_chain(self):
        """R4.1 regression contract."""
        source = ROSLYN_MSG.read_text(encoding='utf-8')
        self.assertIn('for (var current = type; current != null && current.SpecialType != SpecialType.System_Object; current = current.BaseType)', source)
        self.assertIn('var names = new HashSet<string>(StringComparer.Ordinal);', source)
