...
class H07HealthDeadlineTests(unittest.TestCase):
 ...
 def test_read_exact_has_total_deadline(self):
     """R4.1 regression contract."""
  t=SRC.read_text(encoding='utf-8'); self.assertIn('var deadline = Stopwatch.StartNew();',t); self.assertIn('stream.ReadTimeout =',t); self.assertIn('Timed out reading ROS2 Bridge health response.',t)
