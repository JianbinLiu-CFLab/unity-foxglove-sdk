...
class H04NodeLifetimeTests(unittest.TestCase):
    ...
    def test_jazzy_holds_mutex_during_native_action(self):
        """R4.1 regression contract."""
        text = JAZZY.read_text(encoding='utf-8')
        self.assertIn('Hold the mutex through the native action', text)
        section = text[text.index('private TResult WithLiveNode'):text.index('/// <summary>', text.index('private TResult WithLiveNode'))]
        self.assertIn('lock (mutex)', section)
        self.assertIn('return action(node);', section)

    def test_humble_control_also_holds_mutex(self):
        """R4.1 regression contract."""
        text = HUMBLE.read_text(encoding='utf-8')
        section = text[text.index('private TResult WithLiveNode'):text.index('/// <summary>', text.index('private TResult WithLiveNode'))]
        self.assertIn('lock (mutex)', section)
        self.assertIn('return action(node);', section)
