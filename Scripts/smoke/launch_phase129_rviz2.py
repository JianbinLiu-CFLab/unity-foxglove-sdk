...
# Purpose: Launch Phase129 RViz2 with the Windows Jazzy runtime environment.

"""Launch RViz2 for the PointCloud2 manual acceptance.
...
to the live `/tf` and `/points` streams.
"""
...
def main(argv: list[str]) -> int:
    ...
    print(f"[phase129-launch] ROS2 root: {ros2_root}")
    print(f"[phase129-launch] pixi Python: {pixi_python}")
    print(f"[phase129-launch] ros2-script.py: {ros2_script}")
    print(f"[phase129-launch] ROS_DISTRO: {env.get('ROS_DISTRO')}")
    print(f"[phase129-launch] RMW_IMPLEMENTATION: {env.get('RMW_IMPLEMENTATION')}")
    print(f"[phase129-launch] ROS_DOMAIN_ID: {env.get('ROS_DOMAIN_ID')}")
    print(f"[phase129-launch] ROS_AUTOMATIC_DISCOVERY_RANGE: {env.get('ROS_AUTOMATIC_DISCOVERY_RANGE', '<unset>')}")
    print(f"[phase129-launch] RViz2 config: {rviz_config}")

    if args.dry_run:
        print("[phase129-launch] Dry run only; RViz2 was not launched.")
        return 0

    ros2env.launch_rviz(ros2_root, rviz_config, env, "phase129-launch")
    print("[phase129-launch] Confirm RViz2 displays TF and PointCloud2 /points.")
    return 0
...
if __name__ == "__main__":
    try:
        ...
        print(f"[phase129-launch] FAIL: {exc}", file=sys.stderr)
        raise SystemExit(1)
