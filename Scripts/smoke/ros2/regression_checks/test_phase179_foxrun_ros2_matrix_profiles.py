...
class Phase179FoxRunRos2MatrixProfileTests(unittest.TestCase):
    ...
    def test_windows_editor_root_is_repo_local_and_all_three_endpoints_are_checked_with_array_argv(self) -> None:
        """Editor preflight uses ros2-windows for CLI evidence while leaving Unity's native DLL selection untouched."""
        ...
        for name in profile.message_set:
            ...
            expected_outputs.extend(
                [
                    SimpleNamespace(returncode=0, stdout=f"/foxrun/phase179/{name} [{spec.message_type}]\n"),
                    SimpleNamespace(
                        returncode=0,
                        stdout=(
                            f"Type: {spec.message_type}\nSubscription count: 1\nSubscription #0:\n"
                            f"  Reliability: {spec.qos_reliability.upper()}\n"
                            f"  History (Depth): {spec.qos_history.upper()} ({spec.qos_depth})\n"
                            f"  Durability: {spec.qos_durability.upper()}\n"
                        ),
                    ),
                ]
            )
        with mock.patch.object(self.profiles.ros2env, "run_ros2", side_effect=expected_outputs) as run_ros2:
        ...
        self.assertEqual(6, run_ros2.call_count)
        for call in run_ros2.call_args_list:
            self.assertIsInstance(call.args[3], list)
            self.assertNotIn(" ", call.args[3][0])
