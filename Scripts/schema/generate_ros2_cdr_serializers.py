...
from generate_ros2_msg_schema_catalog import CATEGORIES, CSHARP_NAMESPACE, EXPECTED_FILE_COUNT
...
def reader_for_scalar(field: Field) -> str:
    ...
    if field.base_type == "geometry_msgs/Point":
        return "ReadProtoPoint(reader)" if field.property_type == "global::Foxglove.Point3" else "ReadProtoVector3(reader)"
    if field.base_type == "geometry_msgs/Vector3":
        return "ReadProtoVector3(reader)"
    if field.base_type == "geometry_msgs/Quaternion":
        return "ReadProtoQuaternion(reader)"
    if field.base_type == "geometry_msgs/Pose":
        return "ReadProtoPose(reader)"
    if field.base_type.startswith("foxglove_msgs/"):
        return f"Read{field.base_type.split('/', 1)[1]}(reader)"
    raise RuntimeError(f"Unsupported scalar field: {field}")


def reader_for_field(field: Field) -> list[str]:
    """Generate C# reader statements for one scalar, sequence, or fixed field."""

    access = f"message.{field.property_name}"
    if field.array_kind == "scalar":
        return [f"{access} = {reader_for_scalar(field)};"]
    if field.base_type == "uint8":
    ...
    if field.base_type == "uint32":
        return [f"{access}.Add(reader.ReadUInt32Sequence());"]
    ...
    if field.base_type == "geometry_msgs/Point":
        ...
        item_reader = f"Read{field.base_type.split('/', 1)[1]}(reader)"
    if item_reader is None:
        raise RuntimeError(f"Unsupported sequence field: {field}")

    count_name = f"{field.property_name}Count"
    return [
        f"var {count_name} = reader.ReadSequenceLength();",
        f"for (var i = 0; i < {count_name}; i++)",
        f"    {access}.Add({item_reader});",
    ]
...
def generate_deserializers(schemas: list[Schema]) -> str:
    """Generate the C# per-schema direct CDR deserializer source."""
    ...
    for schema in schemas:
        lines.extend(
            [
                f"        internal static {schema_type(schema.name)} Read{schema.name}(Ros2CdrReader reader)",
                "        {",
                "            if (reader == null)",
                "                throw new ArgumentNullException(nameof(reader));",
        ...
        for field in schema.fields:
            for line in reader_for_field(field):
                lines.append("            " + line)
...
def generate_deserializer_registry(schemas: list[Schema]) -> str:
    """Generate the C# deserializer registry source."""
    ...
    for schema in schemas:
        ...
        entry_lines.append(
            ...
            f"                {csharp_string(category)},\n"
            f"                {csharp_string(schema.source_file)},\n"
            f"                payload => Ros2CdrGeneratedDeserializers.Deserialize{schema.name}(payload))"
        )
    entries = ",\n".join(entry_lines)
    return f"""// Copyright (c) 2026 Jianbin Liu and Unity2Foxglove contributors.
// SPDX-License-Identifier: Apache-2.0
...
{{
    ...
    {{
        public const int DeserializerCount = {EXPECTED_FILE_COUNT};
...
def generate(input_dir: Path, output_dir: Path) -> str:
    """Generate serializers, deserializers, registries, samples, and Unity .meta files."""
    ...
    outputs = {
        ...
        "Ros2CdrGeneratedDeserializers.g.cs": generate_deserializers(schemas),
        "Ros2CdrSerializerRegistry.g.cs": generate_registry(schemas),
        "Ros2CdrDeserializerRegistry.g.cs": generate_deserializer_registry(schemas),
    ...
    return f"[ros2-cdr-gen] generated {len(schemas)} serializers/deserializers in {output_dir}"
