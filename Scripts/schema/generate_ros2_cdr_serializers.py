...
def writer_for_scalar(field: Field, message_expr: str) -> list[str]:
    ...
    if field.base_type == "geometry_msgs/Point":
        return [f"Ros2CdrGeometryWriter.WritePoint(writer, {access});"]
    if field.base_type == "geometry_msgs/Vector3":
        return [f"Ros2CdrGeometryWriter.WriteVector3(writer, {access});"]
    if field.base_type == "geometry_msgs/Quaternion":
        return [f"Ros2CdrGeometryWriter.WriteQuaternion(writer, {access});"]
    if field.base_type == "geometry_msgs/Pose":
        return [f"Ros2CdrGeometryWriter.WritePose(writer, {access});"]
...
def writer_for_field(field: Field) -> list[str]:
    ...
    item_writer = None
    if field.base_type == "geometry_msgs/Point":
        item_writer = "Ros2CdrGeometryWriter.WritePoint(writer, item);"
    elif field.base_type == "geometry_msgs/Pose":
        item_writer = "Ros2CdrGeometryWriter.WritePose(writer, item);"
