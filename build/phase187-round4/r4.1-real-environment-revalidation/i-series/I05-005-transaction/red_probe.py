...
p=Path('build/phase187-round4/r4.1-real-environment-revalidation/i-series/I05-005-transaction/originals/dotnet-tests.yml')
lines=p.read_text().splitlines(); names=['Run xUnit unit tests','Run FoxRun publish panel behavior tests','Validate source generator DLL freshness script','Validate generated ROS2 schema output freshness','Run validation suite','Run Phase179 ROS2 acceptance helper regressions','Run Phase181 custom ROS2 acceptance helper regressions','Run Phase186 Bridge tooling and package-composition gate','Run official MCAP differential conformance','Validate local entrypoints']
missing=[]
for n in names:
...
print('RED_MISSING_ALWAYS='+str(len(missing))); print('RED_RESULT='+str(bool(missing)))
