...
flowchart LR
  ...
  FoxRun["FoxRun<br/>Generated bidirectional bindings"]
...
| Encoding and FoxRun update rules | Policy / strategy functions | `Runtime/Components/Publishing/FoxglovePublisherEncoding.cs`; `Runtime/Utilities/FoxRunUpdatePolicy.cs` | Keeps wire selection and direction-aware publish/apply scheduling deterministic and testable without creating interface hierarchies. |
...
| Schema and scene conversion | Adapter / builder helpers | `Runtime/Components/Replay/FoxgloveReplayObjectAdapter.cs`; `Runtime/Schemas/Proto/Builders/` | Keeps Unity object models separate from Foxglove schema/wire formats and makes replay state application an explicit boundary. |
...
| FoxRun generation | Shared model + shared emitter | `Editor/Shared/FoxRunDescriptor/`; `Editor/Shared/FoxgloveSourceEmitter/`; `Editor/FoxRun/`; `Editor/SourceGenerators/` | Keeps Roslyn source generation and build-time physical `.g.cs` generation aligned through one semantic model and one emitter. That model drives direct-access publish, subscribe, and full-duplex bindings while preserving one input source, multi-sink output, stable schema identity, and main-thread apply. See [Shared-Emitter Dual-Host AOT Code Generation for Bidirectional Unity Telemetry](research-shared-emitter-architecture.md). |
...
- FoxRun direction-aware update policy;
...
- **FoxRun:** start with `Editor/Shared/FoxRunDescriptor/` and `Editor/Shared/FoxgloveSourceEmitter/`, then inspect both Roslyn/build-time hosts and the runtime input/output routing under `Runtime/Components/FoxRun/`.
...
For research positioning and related work, see [Research Related Work Evidence](research-related-work-evidence.md). For the FoxRun code-generation architecture, see [Shared-Emitter Dual-Host AOT Code Generation for Bidirectional Unity Telemetry](research-shared-emitter-architecture.md).
